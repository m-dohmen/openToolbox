...
6. `npm run build && npm test`. Alle zwölf Suiten müssen laufen; vorab prüft `scripts/check-suite-drift.js` als CI-Tor, dass die fünf Aufzählungsstellen mit `scripts.test` übereinstimmen (Reihenfolge und Vollständigkeit).
