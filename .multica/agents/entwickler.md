...
6. `npm run build && npm test`. Alle zwölf Suiten müssen laufen; die Aufzählung wird in der CI durch `scripts/check-suite-drift.mjs` gegen `scripts.test` geprüft (OPEN-125) — Drift fällt die Pipeline, ohne dass `npm test` überhaupt startet.
