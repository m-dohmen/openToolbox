...
`npm test` drives a real headless browser against the built file over `file://`. If your change
touches persistence, encryption, the AI client or branding, add an assertion to `test/smoke.mjs`.
A feature without a test tends to break silently three commits later, because nobody clicks through
a single-file app by hand.
