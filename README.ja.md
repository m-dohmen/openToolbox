...
- **エンティティ単位のカンバン。** スキーマの `view.board` 宣言で有効化——カードをマウス／タッチ／キーボードで列間移動でき、
  すべての移動はフォームと同じ書き込み経路を通るため、元に戻す／やり直しと変更履歴に同時に残ります。
...
- **ダッシュボードのインライン SVG グラフ**（棒、環、線）— `DASHBOARD.charts` 配列で統一形
  `{ type: 'chart', kind: 'bar'|'donut'|'line' }` を宣言。`line` は月単位（`count` または `sum(フィールド)`）で集計し、
  空の月は飛ばす。すべてインライン SVG、チャートライブラリなし、オフラインでも動く。印刷用スタイルは白黒の
  破線に切り替わる。タイル・指標カード・明暗両テーマと共存。
