    ...
    seed: () =>
      [
        ['Core banking migration', 'Sparkasse Rheinland', 'A. Reinke', 'Delivery', 'high', 1250, 890, -120, 95,
          'Cutover window needs regulator sign-off before the rehearsal is scheduled.'],
        ...
        ['Regulatory reporting overhaul', 'Landesbank Sued', 'T. Krueger', 'Rollout', 'high', 2100, 1980, -300, -8,
          'BaFin asked for a revised format sample; parallel run extended by two weeks.'],
        ['Cloud migration wave 2', 'Sparkasse Rheinland', 'M. Voss', 'Initiation', 'low', 460, 35, -20, 210,
          'Scope frozen until the network review.'],
        ...
        ['KYC process automation', 'Direktbank24', 'A. Reinke', 'Rollout', 'medium', 890, 705, -180, 45,
          'Vendor contract covers passport and ID card only; driving licence is phase two.'],
      ].map(([name, client, lead, phase, risk, budget, spent, startDays, endDays, note], i) => ({
        id: 'P-' + String(101 + i),
        name,
        client,
        lead,
    ...
    seed: () =>
      [
        ...
        ['Data migration dry run', 'P-101', 'T. Krueger', 8, 'in progress', 22,
          'Second dry run on the anonymised sandbox extract.'],
        ['Cutover rehearsal', 'P-101', 'A. Reinke', 0, 'open', 18, ''],
        ['SEPA interface certified', 'P-102', 'K. Lorenz', -4, 'waiting', 9,
          'Waiting for the counterpart bank to confirm the test cases.'],
        ...
        ['Parallel run completed', 'P-103', 'T. Krueger', 5, 'in progress', 26,
          'Extended after the BaFin feedback on the format sample.'],
        ['Landing zone provisioned', 'P-104', 'M. Voss', 60, 'open', 14,
          'Firewall rules flagged as a prerequisite by the network team.'],
        ...
        ['ID verification vendor live', 'P-107', 'A. Reinke', 12, 'in progress', 16,
          'Test cases limited to passport and ID card.'],
        ['Manual review queue retired', 'P-107', 'A. Reinke', 48, 'open', 10, ''],
      ].map(([title, projectId, owner, days, status, effort, note], i) => ({
        id: 'M-' + String(201 + i),
        title,
        projectId,
        owner,
